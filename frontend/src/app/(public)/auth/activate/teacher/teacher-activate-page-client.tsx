...
export function TeacherActivatePageClient() {
  ...
  const handleResend = async () => {
    if (resendCooldown > 0) return;
    try {
      await requestMutation.mutateAsync({ email, purpose: "TEACHER_ONBOARDING" });
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-4 text-center">
          ...
          <div>
            <CardTitle className="text-2xl font-bold">Teacher Activation</CardTitle>
            <CardDescription>
              Enter the 6-digit code to verify your educator account
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-sm text-muted-foreground">
            ...
            <span>Check your university email for the activation code</span>
          </div>
