...
async def run(email):
    ...
    if user:
        # Promote
        await conn.execute("UPDATE \"user\" SET role = 'SUPERADMIN' WHERE email = $1", email)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Example: python promote_superadmin.py admin@atlas.tn")
        asyncio.run(run("admin@atlas.tn"))
